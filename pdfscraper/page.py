...
class Page:
    ...
    def split(self, condition: Callable):
        words_true, words_false = self._split_sequence_by_condition(
            self.words, condition
        )
        drawings_true, drawings_false = self._split_sequence_by_condition(
            self.drawings, condition
        )
        images_true, images_false = self._split_sequence_by_condition(
            self.images, condition
        )
        ret_1 = PageSection(
            words=words_true,
            drawings=drawings_true,
            images=images_true,
            parent=self,
            condition=condition,
        )
    ...
    @classmethod
    def from_pymupdf(
        cls, page: "fitz.fitz.Page", orientation: Orientation = None
    ) -> Page:
        ...
        images = [
            Image.from_pymupdf(
                image=image, doc=page.parent, orientation=page_orientation
            )
    ...
    @classmethod
    def from_pdfminer(
        cls, page: "pdfminer.layout.LTPage", orientation: Orientation = None
    ) -> Page:
        import pdfminer

        page_orientation = PageOrientation(
            orientation=orientation, page_width=page.width, page_height=page.height
        )
        if not orientation:
            page_orientation = PageOrientation.create(
                bottom_is_zero=False,
        ...
        words, blocks = _get_words_blocks_from_page(
            page, page_orientation=page_orientation
        )
        page = cls(
            words=words,
            images=images,
