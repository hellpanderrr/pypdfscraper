...
def test_get_span_bbox():
    objs = [
        Word(bbox=Bbox(x0=312.00, y0=531.95, x1=326.16, y1=544.07), text="real"),
...
def test_rectangular():
    ...
    pymupdf_doc = Document.from_pymupdf(
        path=test_path,
        orientation=Orientation.create(bottom_is_zero=True, left_is_zero=True),
    )
    page = pymupdf_doc.pages[0]
    [i.move((100000, 0, 0, 0)) for i in page.images]
    [i.move((100000, 0, 0, 0)) for i in page.words]
    [i.move((100000, 0, 0, 0)) for i in page.drawings]
    print(page.words)
